...
PROCESSED_VIDEOS = set() # Bot ပွင့်နေသရွေ့ ဗီဒီယိုအဟောင်းထပ်မပို့အောင်မှတ်ထားမည်
...
def get_sheet():
    try:
        ...
        # မိမိ Sheet Link ကို ဒီမှာ အမှန်ပြန်ထည့်ပါ
...
async def main():
    ...
    print("✅ Bot is Online with Native RSS & OCR Mode!")

    # --- Telegram Monitoring Section ---
        ...
        for rss_url in yt_rss_links:
            try:
                ...
                for entry in feed.entries:
                    ...
                    # ၁။ Title & Description မှာရှာမယ်
                    desc = entry.summary if 'summary' in entry else ""
                    codes = find_binance_code(entry.title + " " + desc)
                    ...
                    if img_res.status_code == 200:
                        ...
                        for (_, text, _) in READER.readtext('temp.jpg'):
                            codes.extend(find_binance_code(text))
                    
                    # Code တွေ့လျှင် ပို့မည်
